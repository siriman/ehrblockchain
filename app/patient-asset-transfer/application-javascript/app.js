...
exports.registerUser = async function(attributes) {
  ...
  try {
    ...
    if (hospitalId === 1) {
      const ccp = buildCCPHosp1();
      const caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp1.seysoo.com');
      ...
      const caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp2.seysoo.com');
      ...
      const caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp3.seysoo.com');
...
exports.getAllDoctorsByHospitalId = async function(networkObj, hospitalId) {
  ...
  try {
    ...
    if (hospitalId === 1) {
      const ccp = buildCCPHosp1();
      caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp1.seysoo.com');
    } else if (hospitalId === 2) {
      const ccp = buildCCPHosp2();
      caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp2.seysoo.com');
    } else if (hospitalId === 3) {
      const ccp = buildCCPHosp3();
      caClient = buildCAClient(FabricCAServices, ccp, 'ca.hosp3.seysoo.com');
    }
